...
export async function GET(request: NextRequest) {
  try {
    ...
    if (response.ok) {
      ...
      console.log("Short term artists found:", data.items?.length || 0);
      ...
      if (!data.items || data.items.length === 0) {
        ...
        if (response.ok) {
          data = await response.json();
          console.log("Medium term artists found:", data.items?.length || 0);
        }
      }
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch top artists from Spotify", details: error },
        { status: response.status }
      );
    }

    console.log("Final artists data:", data);
